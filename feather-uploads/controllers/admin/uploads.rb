module Admin
  class Uploads < Base
    ...
    def create(upload)
      ...
      @upload.url = (@upload.url[0...1] == "/" ? @upload.url : "/#{params[:file][:filename]}")
      @upload.url = (@upload.url[0...6] == "/files" ? @upload.url : "/files/#{params[:file][:filename]}")
      ...
      unless params[:file].nil? || params[:file][:tempfile].nil?
        ...
        File.chmod(0644, Merb.root+"/public/files/#{params[:file][:filename]}")
      end
