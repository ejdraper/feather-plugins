module Feather
  module Admin
    class PingServices < Base
      ...
      def index
        @ping_services = Feather::PingService.all
        display @ping_services
      end

      def new
        @ping_service = Feather::PingService.new
        display @ping_service
      end

      def create(ping_service)
        ping_service["extended"] = (ping_service["extended"] == "0" ? false : true)
        @ping_service = Feather::PingService.new(ping_service)
      ...
      private
        def find_ping_service
          @ping_service = Feather::PingService[params[:id]]
        end
    end
  end
end