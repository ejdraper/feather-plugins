    ...
    class Typo < Base
      ...
      private
        ...
        def collect_typo_articles()
          ...
          puts "Article importing started"
          TypoArticle.find_by_sql("select * from contents where type = 'Article'")
        end
        ...
        def process_articles(typo_articles, permalink_format)
          ...
          typo_articles.each do |a|
            ...
            if is_plugin_active("feather-tagging") && defined?(Tag) && defined?(Tagging) && article.respond_to?("tag_list=")
              ...
              DataMapper.database(:typo_database) do
                taggings = TypoTagging.find_by_sql("select * from articles_tags where article_id = #{a.id}")
                tags = TypoTag.find_by_sql("select * from tags where id in (#{taggings.collect{|tg| tg.tag_id}.join(',')})") unless taggings.empty?
            
                assigned_sections = TypoAssignedSection.find_by_sql("select * from categorizations where article_id = #{a.id}")
                sections = TypoSection.find_by_sql("select * from categories where id in (#{assigned_sections.collect{|as| as.category_id}.join(',')})") unless assigned_sections.empty?
              end
        ...
        def collect_typo_comments()
            puts "Comment importing started"
          TypoComment.find_by_sql("select * from feedback where type = 'Comment'")
        end
